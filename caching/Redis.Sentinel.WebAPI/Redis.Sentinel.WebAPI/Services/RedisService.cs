...
namespace Redis.Sentinel.WebAPI.Services;

public class RedisService
{
    private static ConfigurationOptions sentinelOptions => new()
    {
        EndPoints =
        {
            { "localhost", 6383 },
    ...
    private static ConfigurationOptions masterOptions => new()
    {
        AbortOnConnectFail = false,
    };
    ...
    public static async Task<IDatabase> RedisMasterDatabase()
    {
        ...
        string? localMasterIp = masterEndPoint?.ToString() switch
        {
            ...
            _ => throw new Exception("Master not found")
        };
